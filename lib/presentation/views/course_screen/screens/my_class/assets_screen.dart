...
class _AssetsScreenState extends ConsumerState<AssetsScreen> {
  ...
  Widget build(BuildContext context) {
    final assets = ref.watch(getClassDetailsProvider);
    final data = assets.value?.data;
    final classAssets = data?.classAssets;
    final videos = classAssets?.videos ?? [];
    final pdfs = classAssets?.pdfs ?? [];
                            ...
                            : ListView(
                                ...
                                children: pdfs.map((pdfModule) {
                                  return Padding(
                                    padding: EdgeInsets.only(bottom: 8.h),
                                    child: AssetsWidget(
                                      title: pdfModule.fileName ?? 'N/A',
                                      isVideo: false,
                                      hasIcon: false,
                                      onTap: () {
                                        logger.d(
                                          "Pdf url : ${pdfModule.assetUrl}",
                                        );
                                        logger.d(
                                          "Pdf file name : ${pdfModule.fileName}",
                                        );
                                        Navigator.pushNamed(
                                          context,
                                          RouteNames.pdfViewerScreen,
                                          arguments: {
                                            'asset_url': pdfModule.assetUrl,
                                            'file_name': pdfModule.fileName,
                                          },
                                        );
                                      },
                                    ),
