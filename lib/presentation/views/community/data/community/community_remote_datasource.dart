...
class CommunityRemoteDataSource {
  ...
  Future<List<GetFeedModel>> getCommunityFeeds() async {
    try {
      final response = await apiClient.get(ApiEndpoints.getFeed);
      ...
      if (response.data is List) {
        ...
        // Log raw poll_options from first item
        if (dataList.isNotEmpty) {
          logger.d('First item raw: ${dataList[0]}');
          logger.d('poll_options field: ${dataList[0]['poll_options']}');
        }
